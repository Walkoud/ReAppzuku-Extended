[English](./README.md) | **Русский** | [简体中文](./README_ZH.md) | [Espanol](./README_ES.md) | [Українська](./README_UK.md) | [Deutsch](./README_DE.md) | [Français](./README_FR.md)
...
## ⭐ Лучшее использование (мои предпочтения)

Это моя личная рекомендуемая настройка для максимальной экономии заряда батареи с минимальными компромиссами.

### Шаги настройки

1. **Включить фоновый сервис** — Настройки → Автоматизация → включите "Фоновый сервис". Это гарантирует, что ReAppzuku сможет применять ограничения даже после выхода из настроек.

2. **Включить шаблон установки** — Настройки → Расширенные инструменты → App Install Template → ВКЛ, затем:
   - Отметьте **Фоновое ограничение** → установите **HARD**
   - Отметьте **Уведомлять при применении** — отправляет уведомление при установке нового приложения.

3. **Массовая установка HARD режима для всех пользовательских приложений:**
   - Перейдите в **Фоновые ограничения** → нажмите **Сортировка** → выберите **Пользовательские**.
   - Нажмите **Выбрать все** (вверху справа).
   - Нажмите **Тип** (вверху справа) → выберите **Hard** → Подтвердить.

4. **Исключения — приложения, которым нужны уведомления или виджеты:**
   - **Снимите выбор** или переключите на **Soft** для:
     - Мессенджеров (WhatsApp, Signal, Telegram, Discord) — Soft сохраняет мгновенные FCM-уведомления.
     - Приложений с виджетами на главном экране.
     - Любых приложений, где нужны push-уведомления.

### Таблица режимов ограничений

| Режим | Что делает технически (через Android/Shizuku) | Зачем использовать? | Влияние на уведомления | Примеры приложений |
|---|---|---|---|---|
| 🟢 SOFT | • Отключает только RUN_ANY_IN_BACKGROUND.<br>• Оставляет приложение пассивно в RAM. | • Предотвращает автозапуск приложения.<br>• Максимальная плавность (без перезагрузки при переключении). | Мгновенные (100% рабочие через FCM Google). | WhatsApp, Signal, Discord, Gmail, Uber. |
| 🟡 MEDIUM | • Отключает 6 ключевых AppOps (ACCESS_NOTIFICATIONS, GET_USAGE_STATS и др.).<br>• Устанавливает Standby Bucket RARE. | • Блокирует отслеживание и слежку.<br>• Разрешает музыку/GPS в фоне при активном использовании. | Заблокированы или сильно задержаны (только пакетами). | Instagram, X (Twitter), Spotify, YouTube, Chrome/Firefox. |
| 🔴 HARD | • Отключает все 11 доступных AppOps (включая WAKE_LOCK, START_FOREGROUND, SCHEDULE_EXACT_ALARM).<br>• Standby Bucket RESTRICTED.<br>• Один force-stop при активации. | • Максимальная защита батареи.<br>• Полная остановка приложения при сворачивании.<br>• Нет паразитных процессов ночью. | Полностью заблокированы (приложение молчит до открытия). | TikTok, AliExpress, Temu, Shein, Мобильные игры, Банковские приложения. |
...
Форк проекта [gree1d/ReAppzuku](https://github.com/gree1d/ReAppzuku), основанный на [northmendo/Appzuku](https://github.com/northmendo/Appzuku).
<br><br>  
